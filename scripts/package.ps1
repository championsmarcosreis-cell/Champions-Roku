...
function Get-SecretValue([string]$name) {
  ...
  $raw = Get-Content -Raw $inPath
  if ($null -eq $raw) { return "" } # empty file returns $null with -Raw
  $v = $raw.Trim()
...
$devAutoplay = Get-SecretValue 'dev_autoplay.txt'
...
function bundledDevAutoplay() as String
  return "$(Escape-BrsString $devAutoplay)"
end function
"@
...
if ($devAutoplay) { Write-Host 'Injected secret: bundledDevAutoplay()' }
